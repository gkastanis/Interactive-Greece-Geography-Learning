...
for name, data in rivers.items():
    ...
    folium.PolyLine(
        coords,
        popup=f"{name}<br>{description}",
        color='blue',
        weight=2,
        opacity=0.8
    ).add_to(river_group)
    
    # Add marker at river source (last coordinate)
    source_point = coords[-1]
    folium.Marker(
        location=source_point,
        popup=f"<b>Πηγή {name}</b><br>{description}",
        icon=folium.Icon(icon='tint', prefix='fa', color='lightblue')
    ).add_to(river_group)
    
    # Add marker at river mouth (first coordinate)
    mouth_point = coords[0]
    folium.Marker(
        location=mouth_point,
        popup=f"<b>Εκβολές {name}</b><br>{description}",
        icon=folium.Icon(icon='water', prefix='fa', color='blue')
    ).add_to(river_group)
