...
<script>
    ...
    function startExploreMode() {
        gameMode = 'explore';
        const modeButton = document.getElementById('mode-toggle-explore');
        modeButton.style.display = 'none';
        document.getElementById('mode-toggle-quiz').style.display = 'block';
        ...
        // Show all markers in explore mode
    ...
    function startQuizMode() {
        gameMode = 'quiz';
        const modeButton = document.getElementById('mode-toggle-quiz');
        modeButton.style.display = 'none';
        document.getElementById('mode-toggle-explore').style.display = 'block';
        // Hide all markers in quiz mode
        document.querySelectorAll('.leaflet-marker-icon').forEach(marker => {
    ...
    function checkAnswer(lat, lng) {
        ...
        if (distance < 50) { // Within 50km radius
            ...
            const correctMarker = document.querySelector(`[data-name="${currentQuestion.name}"]`);
            if (correctMarker) {
                    ...
                    if (!marker.hasClickHandler) {
                        ...
                        marker.addEventListener('click', function(e) {
                            if (gameMode === 'quiz') {
                                ...
                                if (markerLatLng) {
                                    const [lat, lng] = markerLatLng.split(',').map(Number);
                                    checkAnswer(lat, lng);
                                }
                            }
                        });
...
<div style="position: fixed; 
    ...
    background-color: rgba(255, 255, 255, 0.8);
    border-radius: 10px; padding: 10px; z-index: 9999;">
    <button onclick="startExploreMode()" class="game-button explore" id="mode-toggle-explore">🗺️ Εξερεύνηση</button>
    <button onclick="startQuizMode()" class="game-button quiz" id="mode-toggle-quiz" style="display: none;">🎮 Κουίζ</button>
