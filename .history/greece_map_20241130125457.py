...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (map) {
            ...
            // Add click handlers to all markers after they're created
            setTimeout(() => {
                document.querySelectorAll('.leaflet-marker-icon').forEach(marker => {
                    marker.addEventListener('click', function(e) {
                        if (gameMode === 'quiz') {
                            ...
                            const markerLatLng = markerElement.getAttribute('data-latlng').split(',');
                            checkAnswer(parseFloat(markerLatLng[0]), parseFloat(markerLatLng[1]));
                        }
                    });
                });
            }, 1000); // Wait for markers to be created
        }
    });
</script>
'''
