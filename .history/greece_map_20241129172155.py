...
# Custom icons for different features
mountain_icon = folium.Icon(icon='mountain', prefix='fa', color='darkred')
lake_icon = folium.Icon(icon='water', prefix='fa', color='blue')
river_icon = folium.Icon(icon='stream', prefix='fa', color='lightblue')
...
for river_name, river_data in rivers.items():
    coordinates = river_data[:-1]  # All points except the last element (description)
    description = river_data[-1]   # Last element is the description
    
    # Create a polyline for the river course
    folium.PolyLine(
        locations=coordinates,
        weight=2,
        color='blue',
        opacity=0.8,
        popup=f'<b>{river_name}</b><br>{description}'
    ).add_to(river_group)
    
    # Add river icon at the source (last coordinate in our list is usually the source)
    source_point = coordinates[-1]  # Get the last coordinate (source)
    folium.Marker(
        location=source_point,
        popup=f'<b>Πηγή {river_name}</b><br>{description}',
        icon=river_icon
    ).add_to(river_group)
