...
<script>
    ...
    function selectCategory(category) {
        ...
        feature_group.eachLayer((layer) => {
            if (layer instanceof L.Marker && layer.options.category === category) {
                layer.addTo(map);
            }
        });
    ...
    function generateQuestion() {
        // Filter landmarks based on current category
        ...
        currentQuestion = categoryLandmarks[Math.floor(Math.random() * categoryLandmarks.length)];
        triesLeft = 3;
        
        document.getElementById('question').textContent = 
            `Πού βρίσκεται ${currentQuestion.name}; (${triesLeft} προσπάθειες)`;
    }
